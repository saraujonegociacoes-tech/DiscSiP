...
DROP FUNCTION IF EXISTS public.get_leads_dashboard(timestamptz, timestamptz);
...
-- ── 6. Agregação do dashboard por período (fix do truncamento em 1000) ───────
-- getLeadsData agregava v_lead_progress do período em JS; o PostgREST corta a resposta
-- no "Max Rows" (1000), então as contagens truncavam. Aqui a métrica é contada no banco
-- e devolvida como 1 jsonb — imune ao teto. SECURITY INVOKER: o RLS do chamador vale.
-- O front só pós-processa arrays pequenos (rótulos de fase, cap de canal, ordenação).
-- Espelhado em supabase/migrations/20260707_leads_dashboard_rpc.sql.
