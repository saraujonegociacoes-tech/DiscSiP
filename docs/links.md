...
- [`silencio-de-toque-desligado-por-padrao.md`](discadora-docs/fixes/silencio-de-toque-desligado-por-padrao.md) — **🔴 ago/2026**: em máquina nova a ligação ficava **muda do começo ao fim** e o botão "Som" parecia não pegar (a discagem seguinte rearmava o silêncio). O desmute depende do `cmdCallStart` do `microsip.ini`, e o `setup-hooks.ps1` **aborta em silêncio** quando o MicroSIP está aberto na instalação — mesma falha que deixa o `singleMode=0` de fora e quebra a preditiva. Silêncio virou **opt-in** (`AUTO_MUTE_RING=1`) e o helper abre o som na largada. ⚠️ Versão **não** bumpada: não se distribui pelo auto-update
...
- [`correcao-fase-remarketing-e-reaproveitado.md`](painelleads-docs/fixes/correcao-fase-remarketing-e-reaproveitado.md) — **🔴 set/2026**: contagem de Empréstimo não batia com o Pipefy (165 × 125). Não era a leitura: a fase **`Remarketing` (2284 cards, a maior do pipe) nunca foi cadastrada em `lead_phases`** — criada depois do seed, entrou em silêncio, e o poll do Make perdeu movimentações a ponto de a fase atual de ~1/3 da base ficar congelada (total batia: 6620 × 6618). Cadastra a fase, cria a classe pegajosa **Reaproveitado**, a guarda `v_leads_unknown_phase` e o backfill que regrava fase atual **e** histórico (`phases_history` do Pipefy). **`20260901` + `20260902` aplicadas e backfill rodado (02/set): defasagem de Empréstimo caiu de 109 cards para 2, e os 2 são movimentação ao vivo.** A `20260902` corrigiu o KPI de Reaproveitados (mede por entrada na fase, não por `created_at` — dava 2 num ciclo de 2718). ⏳ Falta a **`20260902b`**: mapeia os **cards excluídos no Pipefy** (`leads.deleted_at` + `v_leads_deleted`) e tira eles de **toda** contagem com um filtro só, em `v_lead_progress`, que é o gargalo por onde todo gráfico passa. ⚠️ Não conserta o poll do Make (fora do repo) — sem isso a defasagem volta a crescer.
