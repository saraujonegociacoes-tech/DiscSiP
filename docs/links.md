...
- [`comparacao-por-dias-uteis.md`](projetopainelceo-docs/updates/comparacao-por-dias-uteis.md) — **04/set:** a variação da aba Financeiro passou a comparar **os mesmos dias úteis** do ciclo anterior, no lugar do mesmo tanto de dias corridos. Corrige dois enganos silenciosos: ciclo em andamento medido contra o ciclo anterior **inteiro**, e recorte livre de 15 dias medido contra 15 dias corridos (9, 10 ou 11 úteis conforme o fim de semana). Sem migration — a régua vive em `lib/period.ts` e a janela anterior é buscada com uma 2ª chamada à mesma RPC, em paralelo
- [`meta-diaria-financeiro.md`](projetopainelceo-docs/updates/meta-diaria-financeiro.md) — **02/set:** a aba Financeiro ganhou o card **Diária** (`meta esperada − realizado ÷ dias úteis restantes`) e as quebras por categoria/departamento/forma **subiram para cima do gráfico**. Nasceu do pedido diário do CEO no grupo ("atualiza os números aqui, junto com a diária"), que saía de três prints. Migration `20260902_ceo_meta_financeira.sql` (tabela `ceo_meta_config` + `get_ceo_meta`/`set_ceo_meta`). ⚠️ Dia útil é seg–sex **sem feriados**, e a meta é **um número só** para todos os períodos
